...
> **状态**: 历史审计快照。当前收尾状态以 `reports/v2.7-finalization-report.md`、`README.md`、`USER_GUIDE.md`、`DEVELOPER_GUIDE.md` 与 `TECHNICAL_GUIDE.md` 为准。生产 UI 当前只承诺 Gemini、Doubao 与 Auto；DALL-E 3 仍为内部实验 profile。
>
> **分析日期**: 2026-06-19  
...
| 架构能力 | UI 体现 | 证据 |
|---------|---------|------|
| 生产 Profile（Gemini/Doubao/Auto） | Profile 下拉选择器 | `#profileSelect` |
...
UI **大体反映了架构**。该历史审计指出过"黑箱化过度"问题；v2.7 收尾后，生产 profile、手动 `auto` 模板、移动布局、toast 与 compare 控件已完成一致化。后续仍建议在卡片元数据中展示 `confidence/source/tier`，提升可解释性。
...
| **版本号显示 `v-`** | 历史位置 | 历史快照中加载前显示 `v-`，加载后显示当时版本号；当前版本以 `package.json` 为准 | ⚪ |

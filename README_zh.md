...
引擎采用**混合检测策略**：
1. **像素特征检测 (优先)**：采用优化版归一化互相关 (NCC) 与 **Top-5 插入排序排名**技术。在大图搜索时性能卓越，即使图片经过裁剪、比例变化或元数据丢失，也能自动找回水印中心。
2. **尺寸规则检测 (辅助)**：根据图像的分辨率条件推定默认位置，支持基于阈值的动态 48px/96px 切换。
3. **安全回退机制**：当 Web Worker 指令失败或 Buffer 被意外转移时，引擎会自动通过克隆数据回滚至主线程执行，确保处理流程 100% 成功。

| 图像尺寸条件 | 目标水印尺寸 | 对齐方式 | 默认边距 |
|------------|---------|--------|--------|
| 宽 > 1024 **或** 高 > 1024 | 96×96 | 右下角 | 64px |
| 其他情况 | 48×48 | 右下角 | 32px |

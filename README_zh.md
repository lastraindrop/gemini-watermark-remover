...
- ✅ **强大混合探测 (v1.5)** - 结合尺寸规则、**NCC (归一化相关性)** 及 **自适应降噪**，即使在低画质或部分切边的图像中也能精准定位。
- ✅ **切边容错恢复** - 智能识别并去除超出图像边界（部分被裁剪）的水印。
- ✅ **批量与目录模式** - 支持多文件上传及带有限流保护的全自动本地目录批处理。
- ✅ **安全加固** - 内置 XSS 防御、内存泄漏主动防护及 Web Worker 稳健回退机制。
- ✅ **开发者友好** - 完整的单元测试套件 (node:test)、标准化的测试工具集及 CI/CD 集成。

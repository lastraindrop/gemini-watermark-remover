...
- ✅ **分级识别反馈 (Tier Badge)** - 动态回显 Catalog 匹配状态，100% 确认官方 Tier 分辨率。
- ✅ **跨平台参数对齐** - Web、CLI 与 Python 桌面端 100% 共享 v1.5.5 高级引擎参数协议。
- ✅ **切边容错恢复** - 智能识别并去除超出图像边界（部分被裁剪）的水印。
- ✅ **批量与目录模式** - 支持多文件上传及全自动本地目录批处理。
- ✅ **多语言支持 (5国语言)** - 完整支持 **中、英、日、俄、法** 五国语言及自动识别。
...
├── tests/                 # 标准化测试套件 (52+ 测试用例)
│   ├── consistency.test.js # 核心：参数协议动态对齐自动化校验 (Redline)
│   ├── test_utils.js      # 稳健的测试工厂 (支持 21:9 / 噪点模拟)
│   └── ...                # 详细的单元与集成测试

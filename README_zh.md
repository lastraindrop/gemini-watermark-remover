...
# Gemini & Doubao 无损去水印工具 (v2.2.2)

用于检测、分析并去除 Gemini、Doubao 图片中可见 AI 水印的独立 fork 生产级客户端工具。v2.2.2 包含五阶段检测管线、三维评分（max策略保留高NCC）、自适应多尺度搜索、多遍安全移除、Alpha 增益校准、缩放匹配门控与决策分阶系统。前端已实现零外部CDN/零Google Fonts依赖的完全本地化部署，支持暗色模式手动切换。

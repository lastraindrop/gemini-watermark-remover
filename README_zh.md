...
- ✅ **稳健混合检测** - 结合尺寸规则与**像素互相关 (Pixel Correlation)** 算法，即使 EXIF 丢失也能精准定位
...
引擎采用**混合检测策略**：
1. **像素特征检测 (优先)**：在图像右下角范围内进行滑动窗口互相关计算。即使图片经过裁剪或元数据丢失，也能自动找回水印中心。
2. **尺寸规则检测 (回退)**：根据图像的分辨率条件推定默认位置。

| 图像尺寸条件 | 目标水印尺寸 | 对齐方式 | 默认边距 |
|------------|---------|--------|--------|
| 宽 > 1024 **且** 高 > 1024 | 96×96 | 右下角 | 64px |
| 其他情况 | 48×48 | 右下角 | 32px |

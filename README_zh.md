...
- ✅ **稳健混合检测** - 结合尺寸规则与 **NCC (归一化互相关)** 高精度算法，配合 Top-5 插入排序优化，即使图片经过裁剪或比例变化也能精准定位
- ✅ **高性能批处理** - 支持多文件上传及高性能并行命令行处理
- ✅ **安全加固** - 内置 XSS 防御、内存泄漏自动释放策略以及强健的 Worker 回退机制
- ✅ **开发者友好** - 标准化 Lint/Format，集成 CI/CD，提供完善的 Python Bridge
- ✅ **跨平台支持** - 兼容主流浏览器、Node.js (v18+) 以及 Python 环境
...
引擎采用 **分层混合检测策略 (v1.4)**：
1. **第一层：官方尺寸库匹配 (v1.4 新增)**：将图片尺寸与内置的 **Gemini 官方输出分辨率数据库**（如 512x512, 1024x1024, 1536x672 等）进行比对。如果匹配成功，将直接调用预设的精确锚点，实现秒级锁定。
2. **第二层：标准锚点校验**：在标准边距（32px, 48px, 64px）处使用高精度 NCC 进行快速校验。
3. **第三层：深度 Sobel 扫描 (v1.4 新增)**：深度搜索模式会对模板和图像同时应用 **Sobel 边缘提取算子**。通过匹配“边缘轮廓”而非单纯的“像素亮度”，它能穿透极度复杂的纹理背景（如草地、乱石、复杂风景）定位 Logo。

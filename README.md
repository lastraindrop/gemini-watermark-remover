...
Local-first browser, CLI, SDK, and Python tooling for detecting, analyzing, and removing visible AI watermarks from Gemini and Doubao images. The current production profiles are `gemini`, `doubao`, and `auto`. `dalle3` remains an internal experimental profile and is not advertised as a production UI/CLI target.
...
- **Removal fidelity**: calibrated alpha maps, weak-alpha handling, multi-pass removal for square and rectangular profiles, halo checks, and sub-pixel refinement.
...
| Removal | `src/core/applyRemoval.js`, `src/core/blendModes.js`, `src/core/multiPassRemoval.js`, `src/core/alphaCalibration.js`, `src/core/restorationMetrics.js` | Reverse alpha blending, overlap suppression, multi-pass cleanup, gain calibration, and artifact checks |
...
- Production UI and user docs expose only Gemini, Doubao, and Auto. Experimental profiles must be clearly marked as internal.

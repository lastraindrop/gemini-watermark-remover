# Gemini Watermark Remover - 用户指南 (v2.5.1)

本工具用于对 Gemini、Doubao、DALL·E 3 生成图片进行本地检测、分析与去水印。当前实现包含五阶段检测管线、自适应三维评分、**性能预设系统**、多遍移除与 Alpha 增益校准等增强。
...
当前版本已经增强了近似尺寸、梯度滤波、方差评分改进与矩形水印支持，但极端样本仍可能需要后续继续补 catalog 或阈值样本。
...
|------|---------|---------|------|--------|---------|
| 快速 | 60% | 关闭 | ±3px | 关闭 | 已知尺寸的干净图片 |
| 均衡 | 75% | 开启 | ±6px | 自动 | 大多数场景（默认） |
| 全面 | 90% | 开启 | ±8px | 自动 | 裁剪、缩放、复杂背景 |
